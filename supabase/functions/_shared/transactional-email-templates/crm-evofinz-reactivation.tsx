...
const EvoFinzReactivationEmail = ({ recipientName, body, ruleName }: Props) => (
  <Html lang="es" dir="ltr">
    <Head />
    <Preview>{recipientName ? `${recipientName}, te echamos de menos` : 'Te echamos de menos'} — {SITE_NAME}</Preview>
    <Body style={main}>
      <Container style={wrapper}>
        ...
        <Container style={card}>
          <Heading style={h1}>
            {recipientName ? `${recipientName}, ¿todo bien? 🔥` : '¿Todo bien? 🔥'}
          </Heading>
          <Text style={textStyle}>
            {body || 'Hace tiempo que no sabemos de ti. Tus metas financieras siguen esperándote y hemos mejorado muchas herramientas desde tu última visita. ¿Te gustaría retomar el camino?'}
          </Text>
...
export const template = {
  component: EvoFinzReactivationEmail,
  subject: (data: Record<string, any>) => data.subject || '¿Retomamos tu plan financiero? — EvoFinz',
  displayName: 'EvoFinz Reactivation',
  previewData: { recipientName: 'Carlos', body: 'Han pasado unas semanas desde tu última visita. Hemos agregado nuevas funciones de IA que te pueden ayudar con tu meta de ahorrar más. ¿Volvemos a intentarlo?', ruleName: 'Reactivation Cold' },
